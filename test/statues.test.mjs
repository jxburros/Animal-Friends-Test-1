...
describe('victory', () => {
  test('gaining the 5th statue sets state.winner', async () => {
    ...
    await gainMarketCard(state, 0, 'mk_st_generosity', 'test');
  ...
  test('the 4th statue alone does not end the game', async () => {
    ...
    giveStatue(state, 0, 'mk_st_courage');
    giveStatue(state, 0, 'mk_st_patience');
    await gainMarketCard(state, 0, 'mk_st_harmony', 'test');
    assert.equal(state.winner, null);
  });
});
...
describe('on-gain effects', () => {
  test('Statue of Curiosity: draw 2, discard 1', async () => {
    ...
    await gainMarketCard(state, 0, 'mk_st_curiosity', 'test');
    assert.equal(state.players[0].hand.length, before + 2 - 1);
  });

  test('Statue of Joy: ready up to two Apprentices on gain', async () => {
    const state = newGame();
    const app1 = addStack(state, 0, 'mk_clover_seedling_helper_0', 270); // cost 0 -> apprentice rank, not yet upright
    const app2 = addStack(state, 0, 'mk_patch_junkyard_diver_0', 270); // cost 0 -> apprentice
    const journeyman = addStack(state, 0, 'mk_lynnette_press_feeder_2', 270); // cost 2 -> journeyman, should be ineligible
    state.agents = [{
      choose: async (s, pi, req) => (req.kind === 'pick' && req.reason === 'ready' ? req.options.map((o) => o.uid) : []),
    }, {}];
    await gainMarketCard(state, 0, 'mk_st_joy', 'test');
  ...
  test('Statue of Joy readies at most two Apprentices even if more are eligible', async () => {
    const state = newGame();
    const apps = [
      addStack(state, 0, 'mk_clover_seedling_helper_0', 270),
      addStack(state, 0, 'mk_patch_junkyard_diver_0', 270),
      addStack(state, 0, 'mk_maribel_seed_keeper_1', 270),
    ];
    let offeredMax = null;
    state.agents = [{
  ...
  test('Statue of Generosity: give the opponent 1 Supply and draw 2, then pay its burden on the same gain', async () => {
    ...
    // Giving is a transfer, so the Supply leaves the giver — and leaving is the Statue's own
    // trigger, which hands it straight back. The two cancel out, which is the card working, not a
    // card that gives for nothing.
    assert.equal(state.players[0].supply, meBefore, 'the Supply leaves, and the Statue answers it');
  ...
  test("Statue of Generosity's burden pays the opponent again for every later Statue", async () => {
    ...
    const oppBefore = state.players[1].supply;
    await gainMarketCard(state, 0, 'mk_st_patience', 'test');
...
describe('Statue of Kindness', () => {
  ...
  test('does not trigger when Unemployment is greater than the opponent\'s', async () => {
    ...
    addToUnemployment(state, 0, 'mk_maribel_seed_keeper_1');
    const before = state.players[0].supply;
    await startPhase(state, 0);
