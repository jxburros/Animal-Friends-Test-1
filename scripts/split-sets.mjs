...
// of an animal goes where that animal goes — because an upgrade is only playable over an earlier
// version of the same animal, and a Master whose Apprentice is two releases away is a dead card.
...
out();
out('**How characters move.** All versions of an animal stay together (Apprentice to Master) because an upgrade can only be played over an earlier version of the same animal. Holding back only a Master would leave it with nothing to upgrade over. An Event that needs a named animal goes wherever that animal goes.');
out();
out('## At a glance');
out();
